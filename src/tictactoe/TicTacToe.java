...
public class TicTacToe extends JFrame implements ActionListener{
	...
	protected JButton pion1,pion2,pion3,pion4,pion5,pion6,pion7,pion8,pion9;
	protected JButton[] pions=new JButton[9];
	protected JTextField playerXfield;
	protected JTextField playerOfield;
	protected JLabel playerOlabel;
	protected JLabel playerXlabel;
	protected JLabel turnX,turnY;
	protected int CurrentPlayer=0;
	protected String[] Players=new String[2];
	protected boolean IsGameOver=false;
	protected JLabel GameStatus;
	protected JLabel tictactoeAnimation;
	protected Thread t;
	protected String[] XOstrings={"X","O"};
	protected JLabel playerlabel;
	protected static int player1Score=0,Player2Score=0;
	protected JLabel scorePlayerX,scorePlayerY;
	private JLabel bg;
	...
	public TicTacToe() {
		...
		mntmBackground.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				BackgoundChooser bg=new BackgoundChooser("player");
				bg.setVisible(true);
				setVisible(false);
		...
		mntmHowToPlay.addActionListener(new ActionListener() {
			...
			@Override
			public void actionPerformed(ActionEvent e) {
              How_To_Play abt=new How_To_Play();
              abt.setVisible(true);
			}
		});
		...
		iconPlayerO.setIcon(new ImageIcon("./icons/player2.png"));
		...
		playerOScoreIcon.setIcon(new ImageIcon("./icons/player2.png"));
    ...
    if(!pionValid(pionTarget)){
    	GameStatus.setText("Pion Not Valid !");
    	setAnimation();
    }
    else{
    	setSymbol(pionTarget,XOstrings[CurrentPlayer]);
...
public boolean bourdIsFull() {
	...
	//validation du pion
public	boolean pionValid(JButton pion) {
		if(pion.getText().toString().length()==0){
			return true;
		}
		return false;
	}
	//mettre un symbol X ou O sous forme d'un icon
	public void setSymbol(JButton btn,String symbole){
		btn.setText(symbole);
	}
	// tester s'il y a un joueur qui gagne !!
public boolean WinnerExists() {
	if(pion1.getText()==XOstrings[CurrentPlayer] &&
			pion2.getText()==XOstrings[CurrentPlayer] &&
			pion3.getText()==XOstrings[CurrentPlayer]){
		setWinnerColor(pion1, pion2, pion3);
	...
	if(pion4.getText()==XOstrings[CurrentPlayer] &&
		...
		setWinnerColor(pion4, pion5, pion6);
		score(XOstrings[CurrentPlayer]);
	...
	if(pion7.getText()==XOstrings[CurrentPlayer] &&
		...
		setWinnerColor(pion7, pion8, pion9);
		score(XOstrings[CurrentPlayer]);
	...
	if(pion1.getText()==XOstrings[CurrentPlayer] &&
			pion5.getText()==XOstrings[CurrentPlayer] &&
			pion9.getText()==XOstrings[CurrentPlayer]){
		setWinnerColor(pion1, pion5, pion9);
		score(XOstrings[CurrentPlayer]);
		return true;
	}
	if(pion3.getText()==XOstrings[CurrentPlayer] &&
		...
		setWinnerColor(pion3, pion5, pion7);
		score(XOstrings[CurrentPlayer]);
		return true;
	}
	if(pion1.getText()==XOstrings[CurrentPlayer] &&
			...
			pion7.getText()==XOstrings[CurrentPlayer]){
		setWinnerColor(pion1, pion4, pion7);
		score(XOstrings[CurrentPlayer]);
		return true;
	}
	if(pion2.getText()==XOstrings[CurrentPlayer] &&
			pion5.getText()==XOstrings[CurrentPlayer] &&
			pion8.getText()==XOstrings[CurrentPlayer]){
		setWinnerColor(pion2, pion5, pion8);
		score(XOstrings[CurrentPlayer]);
		return true;
	}
	if(pion3.getText()==XOstrings[CurrentPlayer] &&
			pion6.getText()==XOstrings[CurrentPlayer] &&
			pion9.getText()==XOstrings[CurrentPlayer]){
		setWinnerColor(pion3, pion6, pion9);
		score(XOstrings[CurrentPlayer]);
		return true;
	}
		return false;
...
public void setWinnerColor(JButton pion1,JButton pion2,JButton pion3){
	
	pion1.setForeground(new Color(229,38,128));
	pion2.setForeground(new Color(25,195,69));
	pion3.setForeground(new Color(26,228,188));
}
...
Thread t=new Thread(){
	...
	public void run() {
		try{
			AudioInputStream bgmusic=AudioSystem.getAudioInputStream(new File("./audio/game_music.wav"));
			Clip clip=AudioSystem.getClip();
			clip.open(bgmusic);
			clip.start();
			clip.loop(500);
